...
- ✅ Budgets (2026-09-23): per-agent monthly budgets — `budget.warning` at 80%, `budget.exceeded` at 100% (edge-triggered, at most once each per calendar month), invoke endpoint throttled (403 + `policy.blocked` audit event) when spend is at budget, checked before any provider call; live budget meter on the agent detail page; `budget` block on `AgentDetail` (limit/spend/pct/status). Spend counts only real reported costs (unknown stays out — ADR-0003). 78/78 tests. *(Per-department budgets still open.)*
