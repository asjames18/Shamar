...
- **In progress:** **Phase 4 governance foundations — fourth and final slice done 2026-09-23 ~22:10:** per-department monthly budget pools (ADR-0007) — shared caps via `PUT/GET /api/departments/:name/budget` + `GET /api/departments`; spend summed across member agents per calendar month (real reported costs only); warning 80% / exceeded 100% edge-triggered once per month on the triggering agent's timeline as `department.budget.warning`/`department.budget.exceeded` (distinct types so per-agent budget dedup can never cross-contaminate); invoke gate blocks (403, reason `department_budget_exceeded`) when a pool is exceeded — autonomy still never overrides a hard money cap; dashboard department budgets card with meters + inline set/clear; agent detail shows the pool meter so throttled agents can see why invokes fail. 90/90 tests; E2E verified on a live API. **Phase 4 COMPLETE.** Previously: third slice 2026-09-23 ~21:45 — autonomy-level policy rules (L0–L5) with real server-side enforcement (ADR-0006) — L0 invokes always blocked, L1 needs a human grant inside a trailing 24h, L2 gets max_tokens clamped to 1024, L3/L4/L5 invoke subject to the budget gate (new agents default to L3 Standard); L5 supervisors can grant/deny approvals for agents that list them as `supervisor_agent_id` (decided_by recorded as `agent:<id>`); dashboard agent detail shows a labeled autonomy badge. Previously: second slice 2026-09-23 ~20:30 — per-agent monthly budgets (warning 80% / exceeded 100% edge-triggered alerts, invoke throttling at 403 + `policy.blocked`, live budget meter on agent detail). Previously: first slice 2026-09-23 ~20:00 — approval request/grant/deny workflow is live — `approvals` table, request/grant/deny API with `approval.requested/granted/denied` events in the audit trail, double-decide fails closed, `pending_approvals` on dashboard summary + agent detail, web dashboard card + per-agent Grant/Deny buttons.
