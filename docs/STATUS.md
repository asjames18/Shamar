...
_Last updated: 2026-09-25 ~12:45 EDT.
...
- **PR #11** (`feat/api-sanitize-event-limit`) merged to main (`f1e099e`) — sanitize `GET /api/events?limit=` (good-first-issue 03): non-numeric → **400**.
- **PR #12** (`feat/examples-python-client`) merged to main (`c6ef5b8`) — Python example client (good-first-issue 04).
- **PR #13** (`feat/seed-script-reset-flag`) merged to main (`a5240bd`) — `scripts/seed-demo.js --reset` (good-first-issue 05).
- **This PR:** `feat/dashboard-provider-health` — dashboard Providers section with health badges + Validate (good-first-issue 06).
...
Tests 115/115 — lint clean — typecheck clean — build clean (verified 2026-09-25 ~12:45 EDT; GFI 06 dashboard Providers section only, no test delta).
